...
export function difficultySelector(save: SaveState, language: Language): string {
  ...
  const note =
    save.difficulty === "normal"
      ? language === "en"
        ? "Active: no extra difficulty scaling, no story timer, standard trial energy, untimed duels. Chapter curve still applies (Chapter 9 costs ~1.48x)."
        : "已生效：难度不额外缩放、剧情无时限、试炼精力标准、对决不强制计时；章节曲线仍生效（第 9 章消耗约 ×1.48）"
      : save.difficulty === "pressure"
        ? language === "en"
          ? "Active: 1.4x resource losses, story/duel rounds from 22s (scaled by text length), 1.15x trial energy, more disruptions. Chapter curve stacks (Chapter 9 costs ~2.07x)."
          : "已生效：资源损耗 1.4 倍、剧情/对决 22 秒起（随文本长度增加）、试炼精力 1.15 倍、干扰更多；叠加章节曲线后第 9 章消耗约 ×2.07"
        : language === "en"
          ? "Active: 1.8x resource losses, story/duel rounds from 14s (scaled by text length), 1.3x trial energy, frequent disruptions. Chapter curve stacks (Chapter 9 costs ~2.66x)."
          : "已生效：资源损耗 1.8 倍、剧情/对决 14 秒起（随文本长度增加）、试炼精力 1.3 倍、干扰频繁；叠加章节曲线后第 9 章消耗约 ×2.66";
  return `
